# Claim Context Specification

## Purpose

Defines the `context` block on claims and syntheses: `scope` is required on disk and applied by writers, never inferred by readers, so publish eligibility is decidable from the file alone.
