...
Defines what a workspace and the tool surface say to an agent: the optional conventions document (`dkf.md` or the file named by `workspace.conventions`), its delivery by an MCP server in instructions, prompt, and optionally a resource, the truncation floor, and the register the tool descriptions teach — the subject is the thing in the world, what was read goes in `source.document`.
