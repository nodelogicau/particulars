...
- Interaction with `verifiable-provenance`, now applied: none that belongs in this change. An earlier draft asserted that `held` excludes `kind: defect`, on the grounds that a position has no source to have misread. That is wrong — a held claim's warrant is the holder's position, and a claim can misstate it ("Ben judges the split a mistake" when Ben never judged that), which in an agent-written format is a live failure rather than a hypothetical one. What survives is a constraint independent of the evidential: `kind: provenance-failure` names a cited source that turned out to be wrong, so it has no referent on a claim carrying no `source.document`, whatever its evidential. That belongs with `retraction` and is best raised on its own rather than folded in here.
